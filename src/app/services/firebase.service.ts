...
export class FirebaseService {
  ...
  constructor(private _db:AngularFirestore) {
    this.db = _db.collection('Plc_Estacionamientos');
  }
  ...
  doRegister(){
    return new Promise<any>((resolve, reject) => {
      firebase.auth().createUserWithEmailAndPassword('nguillenv9@gmail.com', 'Jacqueline123.')
      .then(res => {
        resolve(res);
      }, err => reject(err));
    });
  ...
  iniciar(){
    return new Promise<any>((resolve, reject) => {
      firebase.auth().signInWithEmailAndPassword('nguillenv9@gmail.com', 'Jacqueline123.')
      .then(res => {
